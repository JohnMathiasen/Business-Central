/// <summary>
/// Codeunit Calender Message Impl._EVAS (ID 50302).
/// </summary>
codeunit 50302 "Calender Message Impl._EVAS"
{
    var
    ...
    var
        DefaultSubjectTxt: Label 'appointment', Comment = 'DAN="aftale"';
        CreateTxt: Label 'New', Comment = 'DAN="Ny"';
        ModifyTxt: Label 'Update', Comment = 'DAN="Opdatér"';
        CancelTxt: Label 'Cancel', Comment = 'DAN="Aflyse"';
    begin
        if GlobalCalenderEntry.Subject = '' then
            GlobalCalenderEntry.Subject := DefaultSubjectTxt;

        case true of
            (GlobalCalenderEntry."Create Appointment") and (not GlobalCalenderEntry."Cancel Appointment"):
                GlobalCalenderEntry.Subject := StrSubstNo('%1: %2', CreateTxt, GlobalCalenderEntry.Subject);
            (not GlobalCalenderEntry."Create Appointment") and (GlobalCalenderEntry."Cancel Appointment"):
                GlobalCalenderEntry.Subject := StrSubstNo('%1: %2', CancelTxt, GlobalCalenderEntry.Subject);
            (GlobalCalenderEntry."Create Appointment") and (GlobalCalenderEntry."Cancel Appointment"):
                GlobalCalenderEntry.Subject := StrSubstNo('%1: %2', ModifyTxt, GlobalCalenderEntry.Subject);
        end;
    end;
    ...
    // internal procedure GetSubject(IncludeAppointmentType: Boolean): Text
    // var
    ...
    // begin
    //     if IncludeAppointmentType then begin
    //         CreateAppmnt := GetCreateAppointment();
    //         CancelAppmnt := GetCancelAppointment();
    //         case true of
    //             (CreateAppmnt) and (not CancelAppmnt):
    //                 exit(StrSubstNo('%1 %2', CreateTxt, GetSubject()));
    //             (not CreateAppmnt) and (CancelAppmnt):
    //                 exit(StrSubstNo('%1 %2', CancelTxt, GetSubject()));
    //             (CreateAppmnt) and (CancelAppmnt):
    //                 exit(StrSubstNo('%1 %2', ModifyTxt, GetSubject()));
    //         end;
    //     end;
    //     exit(GlobalCalenderEntry.Subject);
    // end;
    ...
    begin
        SetGlobalCalenderEntry();
        GlobalCalenderEntry."Create Appointment" := true;
        GlobalCalenderEntry."Cancel Appointment" := true;
        SetDefaultSubject();
    end;
